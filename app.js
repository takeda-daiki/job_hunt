...
const DEFAULT_SETTINGS = {
  ...
  companyDefaults: {
    scheduledWorkHours: rich(8,7.5,8.5,'assumption'),
...
const VAR_DEFS = {
  scheduledWorkHours:{label:'所定労働時間',unit:'h/日',group:'時間',margin:0.5},
...
const SCENARIO_DEFS = {
  ...
  remittance:{label:'仕送り',unit:'円/月',group:'生活費',money:true}
};
...
function evalNumericCondition(condition,company,scenario){
  const r=metric(company,scenario,condition.variable_key),t=num(condition.threshold),op=condition.operator;
...
function manualEval(condition,company,scenario){
  return state.conditionEvals.find(x=>!x.deleted_at&&x.company_id===company.id&&x.condition_id===condition.id&&(x.scenario_id===scenario?.id||!x.scenario_id))?.status||'unknown';
}
function conditionStatus(condition,company,scenario){ return condition.eval_type==='numeric'?evalNumericCondition(condition,company,scenario):condition.eval_type==='info'?'unknown':manualEval(condition,company,scenario); }
...
async function loadAll(){
  ...
  const [settings,scenarios,companies,values,conditions,evals,notes,attachments]=await Promise.all([
  ...
  state.settings=mergeDeep(structuredClone(DEFAULT_SETTINGS),settings?.settings||{}); state.scenarios=scenarios||[];state.companies=companies||[];state.companyValues=values||[];state.conditions=conditions||[];state.conditionEvals=evals||[];state.notes=notes||[];state.attachments=attachments||[];
}
...
function shell(content,title){
  ...
  return `<div class="app-shell"><aside class="sidebar"><div class="brand">就職条件ノート</div>${nav('companies','企業')}${nav('compare','比較')}${nav('notes','メモ')}${nav('settings','設定')}</aside><main class="main"><div class="topbar"><div class="page-title">${esc(title)}</div><div id="saveStatus" class="save-status ${state.saveState}">${state.saveState==='error'?'⚠ 保存失敗':state.saveState==='saving'?'保存中…':'✓ 保存済み'}</div></div>${content}</main><nav class="bottom-nav">${bnav('companies','企業')}${bnav('compare','比較')}${bnav('notes','メモ')}${bnav('settings','設定')}</nav></div>`;
}
function render(){
  const root=$('#app');
...
function companiesPage(){
  const cards=state.companies.map(c=>{const s=scenarioForCompany(c),t=calcT(c,s),m=calcM(c,s),hs=higherSummary(c,s),prefs=preferencePreview(c,s);return `<div class="card clickable company-card" data-company="${c.id}"><div class="company-head"><div><div class="company-name">${esc(c.name)}</div><div class="small muted">${esc(s?.name||'Scenario未設定')}</div></div><span class="badge info">仮定 ${Math.max(t.assumptionCount,m.assumptionCount)}</span></div><div class="metric-row"><div class="mini-metric"><div class="tiny muted">T_free</div><div class="value mono">${hours(t.central)}</div><div class="tiny muted">${hours(t.low)}〜${hours(t.high)}</div></div><div class="mini-metric"><div class="tiny muted">M_free</div><div class="value mono">${yen(m.central)}</div><div class="tiny muted">${yen(m.low)}〜${yen(m.high)}</div></div></div><div class="small">Higher：未達 ${hs.unmet} / 不確実 ${hs.uncertain} / 不明 ${hs.unknown}</div>${prefs.length?`<div class="pref-preview"><div class="tiny bold">Preference preview</div>${prefs.map(x=>`<div class="small">${statusBadge(x.status)} ${esc(x.c.name)}</div>`).join('')}<button class="pill-link" data-action="open-preference" data-company-id="${c.id}">Preferenceを見る →</button></div>`:''}</div>`;}).join('');
  return `<div class="toolbar"><div class="muted">会社名だけ登録しても仮定値で計算を開始します。</div><button class="btn primary" data-action="add-company">＋企業追加</button></div><div class="grid three">${cards||'<div class="card empty">まだ企業がありません。</div>'}</div>`;
}
...
function overviewPage(c,s){const t=calcT(c,s),m=calcM(c,s),hs=higherSummary(c,s);return `<div class="grid two"><div class="card"><div class="tiny muted">純自由時間 T_free</div><div class="metric mono">${hours(t.central)}</div><div class="metric-sub">想定 ${hours(t.low)}〜${hours(t.high)}</div></div><div class="card"><div class="tiny muted">純自由消費 M_free</div><div class="metric mono">${yen(m.central)}</div><div class="metric-sub">想定 ${yen(m.low)}〜${yen(m.high)}</div></div></div><div class="section-title">Higher condition</div><div class="card"><div class="status-grid"><div class="status-box"><b>${hs.met}</b><span class="tiny">満たす</span></div><div class="status-box"><b>${hs.uncertain}</b><span class="tiny">不確実</span></div><div class="status-box"><b>${hs.unmet}</b><span class="tiny">未達</span></div><div class="status-box"><b>${hs.unknown}</b><span class="tiny">不明</span></div></div><div class="row" style="margin-top:12px"><button class="btn" data-action="goto-tab" data-tab="calc">計算を見る</button><button class="btn" data-action="goto-tab" data-tab="conditions">条件を見る</button><button class="btn" data-action="goto-tab" data-tab="info">情報を見る</button><button class="pill-link" data-action="open-preference" data-company-id="${c.id}">Preferenceを見る →</button></div></div><div class="section-title">企業設定</div><div class="card"><div class="kv"><span>業界</span><b>${esc(c.industry||'未設定')}</b></div><div class="kv"><span>住宅制度</span><b>${esc(({none:'なし',cash:'現金住宅手当',leased:'借上社宅・寮',other:'その他'})[c.settings?.housingType||'none'])}</b></div><div class="row" style="margin-top:12px"><button class="btn" data-action="edit-company" data-company-id="${c.id}">企業情報を編集</button><button class="btn danger" data-action="trash-company" data-company-id="${c.id}">ゴミ箱へ</button></div></div>`;}
...
function calcPage(c,s){const t=calcT(c,s),m=calcM(c,s); const tStatus=t.low>=2?'met':t.high<2?'unmet':'uncertain'; return `<div class="grid two"><div class="card"><div class="tiny muted">純自由時間 T_free</div><div class="metric">${hours(t.central)}</div><div class="metric-sub">想定範囲 ${hours(t.low)}〜${hours(t.high)}</div><hr><div class="kv"><span>出社日</span><b>${hours(t.office)}</b></div><div class="kv"><span>在宅日</span><b>${hours(t.remote)}</b></div><div class="kv"><span>Higher ≥ 2h</span>${statusBadge(tStatus)}</div></div><div class="card"><div class="tiny muted">純自由消費 M_free</div><div class="metric">${yen(m.central)}</div><div class="metric-sub">想定範囲 ${yen(m.low)}〜${yen(m.high)}</div><hr><div class="kv"><span>推定手取り</span><b>${yen(m.breakdown.net)}</b></div><div class="kv"><span>住居費</span><b>${yen(m.breakdown.housing)}</b></div><div class="kv"><span>必須生活費</span><b>${yen(m.breakdown.living)}</b></div><div class="kv"><span>家族関連</span><b>${yen(m.breakdown.family)}</b></div><div class="kv"><span>貯蓄・投資等</span><b>${yen(m.breakdown.future)}</b></div></div></div><div class="section-title">主要変数</div><div class="card detail-list">${mainVarRows(c,s)}</div><div class="row" style="margin-top:12px"><button class="btn primary" data-action="detailed-calc">詳細計算を見る</button></div>`;}
...
function conditionsPage(c,s){
  const renderCat=cat=>{const list=state.conditions.filter(x=>!x.deleted_at&&x.category===cat).sort((a,b)=>a.order_index-b.order_index);return list.length?list.map(cond=>{const st=conditionStatus(cond,c,s);let info='';if(cond.eval_type==='numeric'){const r=metric(c,s,cond.variable_key); const d=VAR_DEFS[cond.variable_key]||{label:cond.variable_key}; info=`<div class="small muted">現在 ${formatVal(cond.variable_key,r.central)} / 範囲 ${formatVal(cond.variable_key,r.low)}〜${formatVal(cond.variable_key,r.high)} / 条件 ${esc(cond.operator)} ${esc(cond.threshold)} ${esc(cond.unit||'')}</div>`;}else if(cond.eval_type==='manual'){info=`<select class="select" data-action="manual-condition" data-condition-id="${cond.id}" style="max-width:170px"><option value="unknown" ${st==='unknown'?'selected':''}>不明</option><option value="met" ${st==='met'?'selected':''}>満たす</option><option value="partial" ${st==='partial'?'selected':''}>一部満たす</option><option value="unmet" ${st==='unmet'?'selected':''}>満たさない</option></select>`;}return `<div class="card" style="margin-bottom:9px"><div class="row" style="justify-content:space-between"><div><b>${esc(cond.name)}</b>${info}</div>${cond.eval_type==='numeric'?statusBadge(st):''}</div></div>`;}).join(''):'<div class="card empty">項目がありません。</div>';};
  return `<div class="tabs">${[['higher','Higher condition'],['preference','Preference'],['information','Information']].map(([k,l])=>`<button class="tab ${state.conditionSubTab===k||(!state.conditionSubTab&&k==='higher')?'active':''}" data-action="condition-subtab" data-subtab="${k}">${l}</button>`).join('')}</div><div id="conditionList">${renderCat(state.conditionSubTab||'higher')}</div><div class="row" style="margin-top:12px"><button class="btn" data-nav="settings" data-settings-section="conditions">条件そのものを管理</button></div>`;
}
...
function comparePage(){
  ...
  let rows=state.companies.map(c=>{const s=scenarioForCompany(c);return {c,s,vals:Object.fromEntries(cols.map(k=>[k,COMPARE_COLUMNS[k]?.value(c,s)]))};});
  const sk=state.compareSort.key;if(sk)rows.sort((a,b)=>((a.vals[sk]??0)-(b.vals[sk]??0))*state.compareSort.dir);
  return `<div class="toolbar"><div class="row"><select class="select" data-action="display-set" style="width:auto">${sets.map(x=>`<option ${x.name===active?.name?'selected':''}>${esc(x.name)}</option>`).join('')}</select><button class="btn" data-action="new-display-set">＋表示セット</button></div><div class="muted small">Scenarioは企業ごとに選択</div></div><div class="table-wrap"><table><thead><tr><th>企業</th><th>Scenario</th>${cols.map(k=>`<th data-action="sort-compare" data-key="${k}">${esc(COMPARE_COLUMNS[k]?.label||k)}</th>`).join('')}</tr></thead><tbody>${rows.map(({c,s,vals})=>`<tr><td><button class="icon-btn bold" data-company="${c.id}">${esc(c.name)}</button></td><td><select class="select" data-action="compare-scenario" data-company-id="${c.id}" style="min-width:130px">${state.scenarios.map(x=>`<option value="${x.id}" ${x.id===s?.id?'selected':''}>${esc(x.name)}</option>`).join('')}</select></td>${cols.map(k=>`<td class="mono">${COMPARE_COLUMNS[k]?.display?COMPARE_COLUMNS[k].display(vals[k]):vals[k]}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
}

function notesPage(){
  let notes=state.notes.filter(n=>state.noteScope==='all'||n.scope_type===state.noteScope); const q=state.noteSearch.trim().toLowerCase();if(q)notes=notes.filter(n=>(n.title+' '+n.body+' '+(n.industry||'')).toLowerCase().includes(q));
  return `<div class="toolbar"><div class="row" style="flex:1"><select class="select" data-action="note-scope" style="width:auto"><option value="all">すべて</option><option value="company" ${state.noteScope==='company'?'selected':''}>企業</option><option value="industry" ${state.noteScope==='industry'?'selected':''}>業界</option><option value="other" ${state.noteScope==='other'?'selected':''}>その他</option></select><input class="input search" data-action="note-search" value="${esc(state.noteSearch)}" placeholder="全文検索"></div><button class="btn primary" data-action="add-global-note">＋メモ</button></div><div class="grid two">${notes.map(n=>{const company=state.companies.find(c=>c.id===n.company_id);return `<div>${n.scope_type==='company'&&company?`<div class="tiny muted" style="margin:0 0 5px 8px">企業：${esc(company.name)}</div>`:n.scope_type==='industry'?`<div class="tiny muted" style="margin:0 0 5px 8px">業界：${esc(n.industry||'')}</div>`:''}${noteCard(n)}</div>`;}).join('')||'<div class="card empty">メモがありません。</div>'}</div>`;
}
...
function companyForm(c={}){return `<form id="companyForm" data-id="${c.id||''}"><div class="form-grid"><div class="form-field full"><label>企業名 *</label><input class="input" name="name" value="${esc(c.name||'')}" required></div><div><label>業界</label><input class="input" name="industry" value="${esc(c.industry||'')}"></div><div><label>住宅制度</label><select class="select" name="housingType"><option value="none">なし</option><option value="cash" ${c.settings?.housingType==='cash'?'selected':''}>現金住宅手当</option><option value="leased" ${c.settings?.housingType==='leased'?'selected':''}>借上社宅・寮</option><option value="other" ${c.settings?.housingType==='other'?'selected':''}>その他</option></select></div><div class="full"><label>採用URL</label><input class="input" name="recruit_url" value="${esc(c.recruit_url||'')}"></div><div class="full"><label>簡単なメモ</label><textarea class="textarea" name="memo">${esc(c.memo||'')}</textarea></div></div><div class="row" style="margin-top:14px"><button class="btn primary">保存</button></div></form>`;}

function variableModal(c,s,key){
  const d=VAR_DEFS[key],ev=state.companyValues.filter(x=>!x.deleted_at&&x.company_id===c.id&&x.variable_key===key&&(x.scenario_id===s.id||!x.scenario_id)); const r=key==='actualRent'?resolveRent(c,s):resolveCompanyVar(c,s,key);
  openModal(d.label,`<div class="card"><div class="kv"><span>現在の採用値</span><b>${formatVal(key,r.central)}</b></div><div class="kv"><span>想定範囲</span><b>${formatVal(key,r.low)}〜${formatVal(key,r.high)}</b></div><div class="kv"><span>状態</span><span>${r.isAssumption?certaintyBadge('assumption'):certaintyBadge(r.certainty||'medium')}</span></div></div><div class="section-title">保存されている情報</div>${ev.length?ev.map(x=>`<div class="card" style="margin-bottom:8px"><div class="row" style="justify-content:space-between"><div><b>${formatVal(key,num(x.central))}</b> ${certaintyBadge(x.certainty)}<div class="small muted">${esc(x.source_label||SOURCE_LABEL[x.source_type])}${x.scenario_id?' / このScenario':''}</div>${x.low!=null||x.high!=null?`<div class="tiny muted">範囲 ${formatVal(key,x.low??x.central)}〜${formatVal(key,x.high??x.central)}</div>`:''}${x.source_url?`<div class="tiny"><a href="${esc(x.source_url)}" target="_blank" rel="noopener">出典を開く</a></div>`:''}</div><div><label class="tiny"><input type="radio" name="primaryEvidence" data-action="make-primary" data-evidence-id="${x.id}" ${x.is_primary?'checked':''}> 中心</label><label class="tiny"><input type="checkbox" data-action="toggle-range" data-evidence-id="${x.id}" ${x.is_range?'checked':''}> レンジ</label><button class="icon-btn tiny" data-action="disable-evidence" data-evidence-id="${x.id}">無効化</button></div></div></div>`).join(''):'<div class="card empty">実情報がないためデフォルト仮定値を使用中。</div>'}<div class="row"><button class="btn primary" data-action="add-evidence" data-key="${key}">＋情報を追加</button></div>`);
}
function evidenceForm(key,s){const d=VAR_DEFS[key];return `<form id="evidenceForm" data-key="${key}"><div class="form-grid"><div><label>値 (${esc(d.unit)})</label><input class="input" type="number" step="any" name="central" required></div><div><label>確証度</label><select class="select" name="certainty"><option value="confirmed">確定</option><option value="high">高</option><option value="medium" selected>中</option><option value="low">低</option><option value="assumption">仮定</option></select></div><div><label>下限（任意）</label><input class="input" type="number" step="any" name="low"></div><div><label>上限（任意）</label><input class="input" type="number" step="any" name="high"></div><div><label>情報源</label><select class="select" name="source_type"><option value="official">公式</option><option value="employee">社員</option><option value="review">口コミ</option><option value="self">自分で設定</option><option value="other">その他</option></select></div><div><label>情報源名</label><input class="input" name="source_label" placeholder="2028卒採用サイト等"></div><div class="full"><label>URL</label><input class="input" name="source_url"></div><div class="full"><label>メモ</label><textarea class="textarea" name="note"></textarea></div><div><label><input type="checkbox" name="is_primary" checked> 中心値に使う</label></div><div><label><input type="checkbox" name="is_range" checked> レンジに使う</label></div><div class="full"><label><input type="checkbox" name="scenario_specific" ${d.scenarioSpecific?'checked':''}> 現在のScenarioだけに適用</label></div></div><div class="row" style="margin-top:14px"><button class="btn primary">保存</button></div></form>`;}
...
function bindEvents(){
  document.addEventListener('click',async e=>{
    const nav=e.target.closest('[data-nav]'); if(nav){state.page=nav.dataset.nav;state.companyId=null;render();return;}
    ...
    if(action==='disable-evidence'){const x=state.companyValues.find(v=>v.id===a.dataset.evidenceId);if(x&&confirm('この情報を計算から無効化しますか？ 履歴は残ります。')){await db(supabase.from('company_values').update({disabled:true}).eq('id',x.id));await loadAll();const c=state.companies.find(z=>z.id===state.companyId),s=scenarioForCompany(c);variableModal(c,s,x.variable_key);}return;}
    ...
    if(action==='manage-conditions'){openModal('条件管理',conditionManager());return;}
    if(action==='add-condition'){openModal('条件追加',conditionForm());return;}
    ...
    if(action==='trash-condition'){await db(supabase.from('conditions').update({deleted_at:nowIso()}).eq('id',a.dataset.conditionId));await loadAll();openModal('条件管理',conditionManager());return;}
  ...
  document.addEventListener('change',async e=>{
    ...
    if(action==='manual-condition'){const c=state.companies.find(x=>x.id===state.companyId),s=scenarioForCompany(c),cond=state.conditions.find(x=>x.id===el.dataset.conditionId);const old=state.conditionEvals.find(x=>x.company_id===c.id&&x.condition_id===cond.id&&!x.deleted_at);if(old)await db(supabase.from('company_condition_evals').update({status:el.value,scenario_id:s?.id}).eq('id',old.id));else await db(supabase.from('company_condition_evals').insert({user_id:state.user.id,company_id:c.id,condition_id:cond.id,scenario_id:s?.id,status:el.value}));await loadAll();render();}
    if(action==='make-primary'){const id=el.dataset.evidenceId,x=state.companyValues.find(v=>v.id===id);await db(supabase.from('company_values').update({is_primary:false}).eq('company_id',x.company_id).eq('variable_key',x.variable_key));await db(supabase.from('company_values').update({is_primary:true}).eq('id',id));await loadAll();const c=state.companies.find(z=>z.id===state.companyId),s=scenarioForCompany(c);variableModal(c,s,x.variable_key);}
    if(action==='toggle-range'){await db(supabase.from('company_values').update({is_range:el.checked}).eq('id',el.dataset.evidenceId));await loadAll();}
  });
  document.addEventListener('input',e=>{if(e.target.dataset.action==='note-search'){state.noteSearch=e.target.value;render();const x=$('[data-action="note-search"]');x?.focus();if(x)x.selectionStart=x.selectionEnd=x.value.length;}});
  document.addEventListener('submit',async e=>{
    const f=e.target;if(!['companyForm','evidenceForm','noteForm','commonSettingsForm','defaultSettingsForm','scenarioEditForm','scenarioAddForm','conditionForm','displaySetForm','importForm'].includes(f.id))return;e.preventDefault();
    try{if(f.id==='companyForm')await submitCompany(f);else if(f.id==='evidenceForm')await submitEvidence(f);else if(f.id==='noteForm')await submitNote(f);else if(f.id==='commonSettingsForm')await submitRichSettings(f,'common',COMMON_DEFS);else if(f.id==='defaultSettingsForm')await submitRichSettings(f,'companyDefaults',VAR_DEFS);else if(f.id==='scenarioEditForm')await submitScenarioEdit(f);else if(f.id==='scenarioAddForm')await submitScenarioAdd(f);else if(f.id==='conditionForm')await submitCondition(f);else if(f.id==='displaySetForm')await submitDisplaySet(f);else if(f.id==='importForm')await submitImport(f);}catch(err){alert(err.message||String(err));}
  });
}

async function submitCompany(f){const fd=new FormData(f),id=f.dataset.id;const payload={user_id:state.user.id,name:fd.get('name'),industry:fd.get('industry')||null,recruit_url:fd.get('recruit_url')||null,memo:fd.get('memo')||null,settings:{housingType:fd.get('housingType')||'none'}};if(id)await db(supabase.from('companies').update(payload).eq('id',id));else{payload.default_scenario_id=state.scenarios[0]?.id||null;await db(supabase.from('companies').insert(payload));}await loadAll();closeModal();render();}
async function submitEvidence(f){const fd=new FormData(f),key=f.dataset.key,c=state.companies.find(x=>x.id===state.companyId),s=scenarioForCompany(c),isPrimary=fd.get('is_primary')==='on';if(isPrimary)await db(supabase.from('company_values').update({is_primary:false}).eq('company_id',c.id).eq('variable_key',key));await db(supabase.from('company_values').insert({user_id:state.user.id,company_id:c.id,scenario_id:fd.get('scenario_specific')==='on'?s.id:null,variable_key:key,central:num(fd.get('central')),low:fd.get('low')===''?null:num(fd.get('low')),high:fd.get('high')===''?null:num(fd.get('high')),certainty:fd.get('certainty'),source_type:fd.get('source_type'),source_label:fd.get('source_label')||null,source_url:fd.get('source_url')||null,note:fd.get('note')||null,is_primary:isPrimary,is_range:fd.get('is_range')==='on'}));await loadAll();variableModal(c,s,key);}
...
function conditionManager(){return `<div class="detail-list">${[...state.conditions].sort((a,b)=>a.order_index-b.order_index).map(c=>`<div class="detail-row"><div><b>${esc(c.name)}</b><div class="tiny muted">${esc(c.category)} / ${esc(c.eval_type)}</div></div><div class="row"><button class="icon-btn" data-action="move-condition" data-condition-id="${c.id}" data-delta="-1">↑</button><button class="icon-btn" data-action="move-condition" data-condition-id="${c.id}" data-delta="1">↓</button></div><div class="row"><button class="btn" data-action="edit-condition" data-condition-id="${c.id}">編集</button><button class="btn danger" data-action="trash-condition" data-condition-id="${c.id}">削除</button></div></div>`).join('')}</div><button class="btn primary" data-action="add-condition" style="margin-top:12px">＋条件追加</button>`;}
function conditionForm(c={}){return `<form id="conditionForm" data-id="${c.id||''}"><div class="form-grid"><div class="full"><label>条件名</label><input class="input" name="name" value="${esc(c.name||'')}" required></div><div><label>分類</label><select class="select" name="category"><option value="higher" ${c.category==='higher'?'selected':''}>Higher condition</option><option value="preference" ${c.category==='preference'?'selected':''}>Preference</option><option value="information" ${c.category==='information'?'selected':''}>Information</option></select></div><div><label>判定</label><select class="select" name="eval_type"><option value="manual" ${c.eval_type==='manual'?'selected':''}>手動</option><option value="numeric" ${c.eval_type==='numeric'?'selected':''}>数値自動</option><option value="info" ${c.eval_type==='info'?'selected':''}>情報のみ</option></select></div><div><label>対象変数（数値時）</label><select class="select" name="variable_key"><option value="tfree" ${c.variable_key==='tfree'?'selected':''}>T_free</option><option value="mfree" ${c.variable_key==='mfree'?'selected':''}>M_free</option>${Object.entries(VAR_DEFS).map(([k,d])=>`<option value="${k}" ${c.variable_key===k?'selected':''}>${esc(d.label)}</option>`).join('')}</select></div><div><label>比較</label><select class="select" name="operator">${['>=','>','<=','<','=','!='].map(op=>`<option value="${op}" ${c.operator===op?'selected':''}>${esc(op)}</option>`).join('')}</select></div><div><label>閾値</label><input class="input" type="number" step="any" name="threshold" value="${c.threshold??''}"></div><div><label>単位</label><input class="input" name="unit" value="${esc(c.unit||'')}"></div></div><button class="btn primary" style="margin-top:14px">保存</button></form>`;}
async function submitCondition(f){const fd=new FormData(f),id=f.dataset.id,max=Math.max(-1,...state.conditions.map(x=>x.order_index||0));const payload={user_id:state.user.id,name:fd.get('name'),category:fd.get('category'),eval_type:fd.get('eval_type'),variable_key:fd.get('eval_type')==='numeric'?fd.get('variable_key'):null,operator:fd.get('eval_type')==='numeric'?fd.get('operator'):null,threshold:fd.get('threshold')===''?null:num(fd.get('threshold')),unit:fd.get('unit')||null};if(id)await db(supabase.from('conditions').update(payload).eq('id',id));else await db(supabase.from('conditions').insert({...payload,order_index:max+1}));await loadAll();openModal('条件管理',conditionManager());}
async function moveCondition(id,delta){const list=[...state.conditions].sort((a,b)=>a.order_index-b.order_index),i=list.findIndex(x=>x.id===id),j=clamp(i+delta,0,list.length-1);if(i===j)return;const a=list[i],b=list[j];await db(supabase.from('conditions').update({order_index:b.order_index}).eq('id',a.id));await db(supabase.from('conditions').update({order_index:a.order_index}).eq('id',b.id));await loadAll();openModal('条件管理',conditionManager());}
...
async function exportJson(){const payload={version:1,exportedAt:nowIso(),appSettings:state.settings,scenarios:state.scenarios,companies:state.companies,companyValues:state.companyValues,conditions:state.conditions,conditionEvals:state.conditionEvals,notes:state.notes,attachments:state.attachments};const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`job-conditions-backup-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(url);alert('JSONを保存しました。添付ファイル本体はSupabase Storageに残り、このJSONには添付メタデータのみ含まれます。');}
...
async function submitImport(f){if(!confirm('JSONバックアップを復元しますか？'))return;const file=new FormData(f).get('file'),obj=JSON.parse(await file.text()),uid=state.user.id;if(obj.appSettings){state.settings=mergeDeep(structuredClone(DEFAULT_SETTINGS),obj.appSettings);await saveSettings();}const upsert=async(table,rows)=>{if(!rows?.length)return;const clean=rows.map(r=>({...r,user_id:uid,deleted_at:null}));await db(supabase.from(table).upsert(clean));};await upsert('scenarios',obj.scenarios);await upsert('companies',obj.companies);await upsert('company_values',obj.companyValues);await upsert('conditions',obj.conditions);await upsert('company_condition_evals',obj.conditionEvals);await upsert('notes',obj.notes);await upsert('attachments',obj.attachments);await loadAll();closeModal();render();}
async function showTrash(){const tables=['companies','scenarios','notes','conditions'];const blocks=[];for(const t of tables){const data=await db(supabase.from(t).select('*').eq('user_id',state.user.id).not('deleted_at','is',null).order('deleted_at',{ascending:false}));if(data.length)blocks.push(`<div class="section-title">${t}</div>${data.map(x=>`<div class="card row" style="justify-content:space-between;margin-bottom:7px"><span>${esc(x.name||x.title||x.id)}</span><button class="btn" data-action="restore-trash" data-table="${t}" data-id="${x.id}">復元</button></div>`).join('')}`);}openModal('ゴミ箱',blocks.join('')||'<div class="empty">ゴミ箱は空です。</div>');}
