...
function overviewPage(c,s){
  ...
  return `<div class="grid two"><div class="card"><div class="tiny muted">純自由時間 T_free</div><div class="metric mono">${hours(t.central)}</div><div class="metric-sub">想定 ${hours(t.low)}〜${hours(t.high)}</div></div><div class="card"><div class="tiny muted">純自由消費 M_free</div><div class="metric mono">${yen(m.central)}</div><div class="metric-sub">想定 ${yen(m.low)}〜${yen(m.high)}</div></div></div><div class="section-title">Higher condition</div><div class="card"><div class="status-grid"><div class="status-box"><b>${hs.met}</b><span class="tiny">満たす</span></div><div class="status-box"><b>${hs.uncertain}</b><span class="tiny">不確実</span></div><div class="status-box"><b>${hs.unmet}</b><span class="tiny">未達</span></div><div class="status-box"><b>${hs.unknown}</b><span class="tiny">不明</span></div></div>${highers.length?`<div class="condition-preview-list">${highers.map(x=>`<div class="condition-preview-row">${statusBadge(x.status)}<span>${esc(x.c.name)}</span></div>`).join('')}</div>`:''}<div class="row" style="margin-top:12px"><button class="pill-link" data-action="open-higher" data-company-id="${c.id}">Higherをすべて見る →</button><button class="pill-link" data-action="open-preference" data-company-id="${c.id}">Preferenceを見る →</button></div></div>${prefs.length?`<div class="section-title">Preference</div><div class="card"><div class="condition-preview-list">${prefs.map(x=>`<div class="condition-preview-row">${statusBadge(x.status)}<span>${esc(x.c.name)}</span></div>`).join('')}</div><button class="pill-link" style="margin-top:10px" data-action="open-preference" data-company-id="${c.id}">Preferenceをすべて見る →</button></div>`:''}<div class="row" style="margin-top:14px"><button class="btn" data-action="goto-tab" data-tab="calc">計算を見る</button><button class="btn" data-action="goto-tab" data-tab="conditions">条件を見る</button><button class="btn" data-action="goto-tab" data-tab="info">情報を見る</button></div><div class="section-title">企業設定</div><div class="card"><div class="kv"><span>業界</span><b>${esc(c.industry||'未設定')}</b></div><div class="kv"><span>住宅制度</span><b>${esc(({none:'なし',cash:'現金住宅手当',leased:'借上社宅・寮',other:'その他'})[c.settings?.housingType||'none'])}</b></div><div class="row" style="margin-top:12px"><button class="btn" data-action="edit-company" data-company-id="${c.id}">企業情報を編集</button><button class="btn danger" data-action="trash-company" data-company-id="${c.id}">ゴミ箱へ</button></div></div>`;
}
...
function bindEvents(){
  ...
  document.addEventListener('submit',async e=>{
    const f=e.target;if(!['companyForm','evidenceForm','salarySplitForm','conditionValueForm','conditionMemoForm','noteForm','commonSettingsForm','defaultSettingsForm','scenarioEditForm','scenarioAddForm','conditionForm','displaySetForm','importForm'].includes(f.id))return;e.preventDefault();
