...
function bindEvents(){
  document.addEventListener('click',async e=>{
    ...
    if(action==='copy-reverse-template'){const t=state.reverseQuestions.find(x=>x.id===a.dataset.templateId);if(t){const max=Math.max(-1,...state.reverseQuestions.filter(x=>x.company_id===a.dataset.companyId).map(x=>x.order_index||0));await db(supabase.from('reverse_questions').insert({user_id:state.user.id,company_id:a.dataset.companyId,text:t.text,category:t.category,memo:t.memo,selected_for_next:true,order_index:max+1}));await loadAll();render();}return;}
    if(action==='condition-to-reverse'){const cond=state.conditions.find(x=>x.id===a.dataset.conditionId),cid=a.dataset.companyId;if(cond){const max=Math.max(-1,...state.reverseQuestions.filter(x=>x.company_id===cid).map(x=>x.order_index||0));await db(supabase.from('reverse_questions').insert({user_id:state.user.id,company_id:cid,text:`「${cond.name}」について、実際の制度や運用を教えてください。`,category:'条件確認',selected_for_next:true,order_index:max+1}));await loadAll();render();}return;}
    ...
    if(action==='promote-event-question'){const eq=state.interviewEventQuestions.find(x=>x.id===a.dataset.eventQuestionId);if(eq){const max=Math.max(-1,...state.interviewQuestions.map(x=>x.order_index||0));const row=await db(supabase.from('interview_questions').insert({user_id:state.user.id,question:eq.question_text,category:'その他',prep_status:eq.next_answer?'draft':'not_started',answer_60:eq.next_answer||null,order_index:max+1}).select().single());await db(supabase.from('interview_event_questions').update({question_id:row.id}).eq('id',eq.id));await loadAll();render();}return;}
...
async function submitCondition(f){const fd=new FormData(f),id=f.dataset.id,cat=fd.get('category'),max=Math.max(-1,...state.conditions.filter(x=>x.category===cat&&x.id!==id).map(x=>x.order_index||0));const oldCond=id?state.conditions.find(x=>x.id===id):null;const payload={user_id:state.user.id,name:fd.get('name'),category:cat,eval_type:fd.get('eval_type'),variable_key:fd.get('eval_type')==='numeric'?fd.get('variable_key'):null,operator:fd.get('eval_type')==='numeric'?fd.get('operator'):null,threshold:fd.get('threshold')===''?null:num(fd.get('threshold')),unit:fd.get('unit')||null};if(id){if(oldCond&&oldCond.category!==cat)payload.order_index=max+1;await db(supabase.from('conditions').update(payload).eq('id',id));}else await db(supabase.from('conditions').insert({...payload,order_index:max+1}));await loadAll();openModal('条件管理',conditionManager());setTimeout(setupConditionSortables,0);}
...
async function submitEventQuestion(f){const fd=new FormData(f),id=f.dataset.id,qid=fd.get('question_id')||null,master=qid?state.interviewQuestions.find(x=>x.id===qid):null,payload={user_id:state.user.id,event_id:f.dataset.eventId,question_id:qid,question_text:fd.get('question_text')||master?.question||'',performance:fd.get('performance'),actual_answer:fd.get('actual_answer')||null,problem:fd.get('problem')||null,next_answer:fd.get('next_answer')||null};if(id)await db(supabase.from('interview_event_questions').update(payload).eq('id',id));else await db(supabase.from('interview_event_questions').insert(payload));await loadAll();closeModal();render();}
